...
namespace PolynomialCalculator
{
    ...
    public sealed class Polynomial : IEquatable<Polynomial>
    {
        ...
        public bool IsZero { get { return Degree == 0 && Math.Abs(_coeffs[0]) < Epsilon; } }
        ...
        public double this[int power]
        {
            ...
            {
                if (power < 0)
                    throw new ArgumentOutOfRangeException("power", "Степень не может быть отрицательной.");
        ...
        /// </summary>
        public static void DivMod(Polynomial dividend, Polynomial divisor,
                                  out Polynomial quotient, out Polynomial remainder)
        {
            ...
            {
                quotient = new Polynomial(0.0);
                remainder = new Polynomial(dividend._coeffs);
                return;
            }
            ...
            remainder = new Polynomial(remArr);
        }
        ...
        {
            if (n < 0)
                throw new ArgumentOutOfRangeException("n", "Показатель степени должен быть неотрицательным целым.");
        ...
        public static Polynomial Parse(string text)
        {
            ...
            if (parts.Length > MaxInputDegree + 1)
                throw new FormatException(
                    "Слишком много коэффициентов: " + parts.Length +
                    ". Допускается не более " + (MaxInputDegree + 1) +
                    " (степень до " + MaxInputDegree + ").");
        ...
        public static Polynomial FromInputCoefficients(double[] coefficients)
        {
            ...
            if (coefficients.Length > MaxInputDegree + 1)
                throw new ArgumentException(
                    "Слишком много коэффициентов: " + coefficients.Length +
                    ". Допускается не более " + (MaxInputDegree + 1) + ".");
        ...
        public override string ToString()
        {
            ...
            for (int i = _coeffs.Length - 1; i >= 0; i--)
            {
                ...
                {
                    case 0: xPart = ""; break;
                    case 1: xPart = "x"; break;
                    default: xPart = "x^" + i; break;
                }
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                ...
                foreach (var c in _coeffs)
                    hash = hash * 31 + Math.Round(c, 9).GetHashCode();
                return hash;
            }
        }
